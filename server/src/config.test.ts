...
describe("readConfig", () => {
  test("requires the server-only API key and normalizes exact origins", () => {
    ...
    const config = readConfig({ OPENAI_API_KEY: " secret ", ALLOWED_ORIGINS: "https://voice.example/, http://localhost:5173" });
    expect(config.apiKey).toBe("secret");
    expect(config.model).toBe("gpt-realtime-2.1-mini");
    expect(config.allowedOrigins).toEqual(["https://voice.example", "http://localhost:5173"]);
  });

  test("rejects invalid startup limits and origin protocols", () => {
    expect(() => readConfig({ OPENAI_API_KEY: "key", PORT: "3001oops" })).toThrow("PORT");
